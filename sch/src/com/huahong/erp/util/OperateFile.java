...
public class OperateFile {
		...
		if (!file.exists()) {
			...
			for (int i = 0; i < strTempArray.length; i++) {
				...
				if (tempFile.isDirectory()) {
					this.delAllFile(strPath + "/" + strTempArray[i]);		// 先删除文件夹里面的文件
					this.delFolder(strPath + "/" + strTempArray[i]);		// 再删除空文件夹
					flag = true;
				}
			}
		}
	...
	public String readFileInFolder(String strPath){
		...
		if(file.isFile() || !file.exists()){
			...
			//读取文件夹内的文件名称
			for(int i = 0; i < files.length; i++) {// 先列出目录
